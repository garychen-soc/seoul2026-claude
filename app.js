...
const TRIP = {
  ...
  bus: {
    ...
    busWarn: "換票櫃台只開到 22:00！你 20:45 落地，務必趕在 22:00 前到 T1 換票處換好 4 人實體票。逾 22:00 即使還有 22:59 末班車，沒換到票也上不了車 → 直接改叫 Kakao T。",
    segs: [
      {
        dir: "去程 · 機場 → 市區（6/30 TW670 抵 T1）",
        time: "首班 05:19／末班 22:59，約 10–20 分一班",
        board: "T1 換票後於 1 樓站牌上車；約 60–75 分到明洞",
        note: "關鍵是「換票」而非「末班車」：換票櫃台 22:00 關，務必 22:00 前換好票；出關慢就直接改 Kakao T", flag: true,
        pick: "換好票後可搭：21:24 · 21:39 · 21:55（建議搭這幾班，預留換票時間）· 之後 22:11 · 22:27 · 22:43 · 22:59(末班)",
    ...
    exchange: [
      ...
      "⏰ 換票處營業 06:00–22:00（去程務必 22:00 前換好票，逾時搭不了車）。換票處：T1 第1航廈 1 樓 6 號門前（內側）；T2 第2航廈 B1 西側停車場巴士售票處。",
...
function viewHome() {
  ...
  bs.innerHTML = `<h2 class="card-h"><span class="ic">🚌</span>${esc(B.title)}</h2>
    ...
    ${B.busWarn ? `<div class="warn bus-deadline">⏰ ${esc(B.busWarn)}</div>` : ""}
